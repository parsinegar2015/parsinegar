...
class pagination{
	...
	private function htmlLink($links,$n,$next,$previous){
		...
		$previous = $this->navigation($previous,'&lt&ltprevious',$url,$template,$info);
		if($previous){
		 $ret[] = $previous;
		}
		$first = false;
		...
		if($link_num > $links[$ix]){
		 $link_num = $links[$ix];
            ...
            $next = $this->navigation($next,'next&gt&gt',$url,$template,$info);
			if($next){
			$ret[] = $next;
			}
		 if(!empty($ret)){
	...
	private function options($options){
			...
			$ret['url'] = (is_array($options) && isset($options['url']) && !empty($options['url']))? $this->baseUrl($options['url']."?page={number}") : $this->baseUrl("?page={number}",0);
